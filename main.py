...
class BioImage:
    ...
    def save_2d_mask(self, po):
        ...
        # save images
        try:  #if cellmass is empty this will avoid crash
        ...
        with open(file_stem + '_2Dresults.csv', 'w') as f:
            ...
            for i in range(self.mask_props.id.size):
                    ...
                    #skip if tf_sat is empty w/ print error
                    if len(self.mask_props.tf_sat.shape) < 2 or self.mask_props.tf_sat.shape[1] == 0:
                        print("tf_sat is not 2D or the second dimension is empty.")
                        return
...
def process_file(po):
    ...
    if np.all(image.im == 0) and not image.big_image:
    ...
    if SO.segment_2D or image.im.shape[1] == 1:
        ...
        # plot
        #image.plot_2d_mask()
        #This function doesn't seem to be doing anything because plotshow is commmented out
...
def scripting():
    ...
    files = find_all_images_in_path(r"C:\Users\Ben\Downloads\A81 16 Weeks BMSRAD70 Ki67 BL54_D111 scRNA\ki67", '.tif')
    model_choice = r"C:\Users\Ben\Documents\RA PAPER IHC CELLPOSE\cellseg\Cellpose Models\CP_nrl_crx"

    diam = 12 # autofit, or change for purpose
    flow_threshold = 0.9  # defaults ~ 0.1 to 3.0 (fewer to more cells), comment if not needed 0.9
    cellprob_threshold = 0.0001  # defaults ~ -6 to 6 (more to fewer pixels), comment if not need 0.0001
    alter_mask_channel = True

    #""" I think the inputs here 0-3 are: 0 for gray, 1 for red, 2 for green, 3 for blue """
    mask_ch = [2,3]# this might accept two channels like this in a list, but a len(1) int should work as well
    #   ch00 = blue = 3
    #   ch01 = green = 2
    #   ch02 = red = 1
